...
*/
func loadAutoCompleteData(settings *Settings) []AutocompleteTreffer {
	// potentially unsafe. However whoever can edit the settings also can see the credentials.
	queryStr := fmt.Sprintf("SELECT %s,%s,%s FROM %s", settings.IdColName, settings.QueryColName, settings.ResultCountColName, settings.TableName)
